...
function renderSummary() {
  ...
  // Settle month (admin only)
  container.querySelector('#settle-period-btn')?.addEventListener('click', async (ev) => {
    const btn = ev.currentTarget;
    const period = btn.dataset.period;
    const { rows, unresolved } = computeStandings(period);
    const msg = currentLang === 'th'
      ? `ปิดยอด ${periodLabel(period, 'th')}?\n\nจะบันทึก standings ${rows.length} คน แล้ว "ย้าย" สลิปของเดือนนี้ไปที่แท็บ slips_archive (ย้าย ไม่ใช่ลบ)`
        + (unresolved ? `\n\nสลิปที่ยังไม่รู้ผล ${unresolved} ใบ จะไม่ถูกย้าย` : '')
      : `Settle ${periodLabel(period, 'en')}?\n\nWrites ${rows.length} standings rows, then MOVES this month's slips to slips_archive.`;
    if (!confirm(msg)) return;

    btn.disabled = true;
    showLoading();
    try {
      const result = await settlePeriod(period, rows, true);
      if (result && result.success) {
        state.periodSlips = [];
        state.standings = [];
        showToast(currentLang === 'th'
          ? `ปิดยอดแล้ว — ย้าย ${result.archived_slips} สลิปเข้าคลัง`
          : `Settled — ${result.archived_slips} slips archived`);
        await renderSummaryLazy();
      } else {
        showToast((result && result.error) || (currentLang === 'th' ? 'ปิดยอดไม่สำเร็จ' : 'Settle failed'), 5000);
      }
    } finally {
      btn.disabled = false;
      hideLoading();
...
// Which month the live slips belong to. Settled months are already archived, so
// whatever is left in `slips` is the running period.
function currentPeriod() {
  const slips = state.periodSlips.length ? state.periodSlips : [];
  const p = slips.map(s => s.period).filter(Boolean).sort();
  return p.length ? p[p.length - 1] : GW_PERIOD[state.gw || currentGw()];
}

// Standings rows for one month, from the same resolveSlip() the UI uses.
// Slips still pending (a postponed match, an unplayed fixture) are left out —
// they stay in `slips` and settle in the month they finally resolve.
function computeStandings(period) {
  const players = getPlayers();
  const slips = getPeriodSlips().filter(s => String(s.period) === String(period) && s.status !== 'cancelled');
  const unresolved = slips.filter(s => resolveSlip(s).status === 'pending');

  const rows = players.map(player => {
    const carry = getSeasonCarry(player, period);   // months before this one only
    const mine = slips.filter(s => s.player === player);
    let money = 0, w = 0, l = 0;
    let streak = carry.streakDir * carry.streakVal;

    mine.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).forEach(s => {
      const r = resolveSlip(s);
      if (r.status === 'pending') return;
      money += r.profit;
      if (r.status === 'won') { w++; streak = streak >= 0 ? streak + 1 : 1; }
    ...
    return {
      player: player,
      period: period,
      settled_money: Math.round(money),
      w: w,
      l: l,
      streak: streak,
      season_total: Math.round(carry.money + money),
    };
  }).filter(r => r.w || r.l || r.settled_money);

  return { rows: rows, unresolved: unresolved.length };
}
...
function renderSettlePanel() {
  ...
  let html = `<div class="card" style="padding:10px;margin-bottom:12px">`;
  html += `<div style="font-weight:700;font-size:0.9rem;margin-bottom:6px">🧾 ${lang === 'th' ? 'ปิดยอดเดือน' : 'Settle month'} — ${periodLabel(period, lang)}</div>`;
  html += `<div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:8px">`;
  html += `${lang === 'th' ? 'ผู้เล่นที่มีผล' : 'Players with results'}: ${rows.length}`;
  if (unresolved) html += ` · ${lang === 'th' ? 'ยังไม่รู้ผล' : 'still pending'}: ${unresolved}`;
  html += `</div>`;
  if (unresolved) {
    html += `<div style="font-size:0.78rem;color:var(--secondary);margin-bottom:8px">${lang === 'th' ? 'สลิปที่ยังไม่รู้ผล (เช่น แมตช์เลื่อน) จะไม่ถูกปิดยอด และยังอยู่ในตาราง slips' : 'Unresolved slips (e.g. postponed matches) are excluded and stay in the slips tab'}</div>`;
  }
  html += `<button id="settle-period-btn" data-period="${period}" style="${SUBTAB_ON}">${lang === 'th' ? 'ปิดยอด + ย้ายเข้าคลัง' : 'Settle + archive'}</button>`;
  html += `</div>`;
  return html;
}
...
function renderUserDashboard(player) {
  ...
  if (initialBal !== null) {
    const allSlipsAll2  = getPeriodSlips();
  ...
  const allSlipsAll = getPeriodSlips();
  ...
  const playerSlips = (getPeriodSlips())
  ...
  function getPlayerRoundKey(s) {
    ...
    const thaiMs = kickoffUtc(match.date).getTime() + 7 * 3600 * 1000;
  ...
  allSlips.filter(s => s.player === player && s.status !== 'cancelled').forEach(slip => {
    ...
    const picks = (slip.picks || []).map(p => {
      ...
      if (p.type === 'corner') {
        ...
        const cardLine = p.line || state.cardLines[p.match_id] || '';
        label = `🟨 ใบ${p.pick === 'over' ? (lang === 'th' ? 'สูง' : 'O') : (lang === 'th' ? 'ต่ำ' : 'U')} ${cardLine}`;
        const cardR = result?.card_result;
        if (cardLine && cardR != null && cardR !== '') {
          const outcome = getOUOutcome(parseFloat(cardLine), parseFloat(cardR));
          const res = outcome[p.pick];
          if (res === 'full') resultBadge = '<span class="badge badge-exact">✓</span>';
          else if (res === 'half') resultBadge = '<span class="badge badge-exact">½✓</span>';
          else if (res === 'push') resultBadge = '<span class="badge badge-correct">Push</span>';
      ...
      } else if (p.type === 'champion') {
        const t = TEAMS[p.pick];
        label = `🏆 ${t ? (lang === 'th' ? t.nameTh : t.name) : p.pick}`;
        const winner = typeof getChampionWinner === 'function' ? getChampionWinner() : null;
        if (winner) {
          resultBadge = p.pick === winner ? '<span class="badge badge-exact">✓</span>' : '<span class="badge badge-wrong">✗</span>';
        }
      } else if (p.type === 'ou') {
...
function renderFunLeaderboard() {
  ...
  const stats = players.map(player => {
    ...
    // Settled months live in `standings`, not in pSlips — carry their money and
    // the streak the player was on, otherwise both reset when a month archives.
    const carry = getSeasonCarry(player);

    let wins = 0, losses = 0, totalBet = 0, totalProfit = 0;
    let streakVal = carry.streakVal, streakDir = carry.streakDir; // 1=win day, -1=loss day (by daily net P&L)
    ...
    pSlips.forEach(s => {
      ...
      if (!m0) return;
      const thaiMs = kickoffUtc(m0.date).getTime() + 7 * 3600 * 1000;
    ...
    // totalProfit = this month; seasonProfit adds every settled month before it
    const seasonProfit = totalProfit + carry.money;
  ...
  const sorted = [...active].sort((a, b) => b.seasonProfit - a.seasonProfit);
  const maxProfit = sorted[0]?.seasonProfit;
  const minProfit = sorted[sorted.length - 1]?.seasonProfit;
  ...
  sorted.forEach(s => {
    const b = [];
    if (active.length > 1 && s.seasonProfit === maxProfit && maxProfit > 0) b.push('👑 ราชา');
    if (byWinRate[0]?.player === s.player && s.settled >= 2) b.push('🎯 สไนเปอร์');
    if (s.streakDir === 1 && s.streakVal >= 2) b.push(`🔥 ออนไฟร์ ×${s.streakVal} วัน`);
    if (active.length > 1 && s.seasonProfit === minProfit && minProfit < 0) b.push('🗑️ เผาเงิน');
  ...
  if (leader?.totalProfit > 0) headlineParts.push([
    `${getDisplayName(leader.player)} นำโด่ง +${leader.totalProfit}`,
    `${getDisplayName(leader.player)} กำลังสบาย อย่าไปอิจฉาเลย`,
    `${getDisplayName(leader.player)} ยิ้มอยู่คนเดียว`,
  ][leader.totalProfit % 3]);
  if (hottest?.streakVal >= 2) headlineParts.push([
    `${getDisplayName(hottest.player)} ร้อนแรง 🔥 ${hottest.streakVal} วันติด`,
    `${getDisplayName(hottest.player)} ถูกติดต่อกัน ${hottest.streakVal} วัน ซวยยังไม่ถึงเท่านั้น`,
    `อย่าหยุด! ${getDisplayName(hottest.player)} ติด ${hottest.streakVal} วันแล้ว 🔥`,
  ][hottest.streakVal % 3]);
  if (coldest?.streakVal >= 2) headlineParts.push([
    `${getDisplayName(coldest.player)} หนาวสั่น 🧊 ${coldest.streakVal} วันติด`,
    `${getDisplayName(coldest.player)} แพ้ ${coldest.streakVal} วันติด ไปนอนก่อนเถอะ`,
    `${getDisplayName(coldest.player)} บอกว่าจะเลิก แต่ยังแทงอยู่ 🤣`,
  ][coldest.streakVal % 3]);
  if (last?.totalProfit < -200) headlineParts.push([
    `${getDisplayName(last.player)} ต้องการการบำบัด 💀`,
    `${getDisplayName(last.player)} ขาดทุน ${last.totalProfit} แต่โอเคนะ`,
    `ส่งกำลังใจให้ ${getDisplayName(last.player)} หน่อย 😅`,
  ][Math.abs(last.totalProfit) % 3]);
  const fallbacks = [
    '🗞️ ดราม่ากำลังก่อตัว...',
    '🗞️ ยังเร็วไป อย่าเพิ่งดีใจ',
    '🗞️ ใครจะรวยก่อนกัน รอดูเลย',
    '🗞️ บอลยังเหลืออีกเยอะ อย่าโอ้อวดก่อน',
  ];
  const headline = headlineParts.length
    ? `🗞️ ${headlineParts.join(' · ')}`
    : fallbacks[active.length % fallbacks.length];
  ...
  // Group stats — computed here so groupStatsHtml is ready before html is built
  ...
  const recentSettled = settledAll.filter(s => new Date(s.timestamp).getTime() > oneDayAgo);
  const statsSource = recentSettled.length >= 3 ? recentSettled : settledAll;
  const statsLabel = recentSettled.length >= 3 ? '24 ชม.' : 'ทั้งหมด';
  let groupStatsHtml = '';

  if (settledAll.length > 0) {
    const withResolved = statsSource.map(s => ({ ...s, r: resolveSlip(s) }));
    ...
    const seed = settledAll.length;
    const pick = (arr) => arr[seed % arr.length];

    const winRateRoast = groupWinRate < 40
      ? pick(['แย่กว่าโยนเหรียญมาก 💀', 'เดากันมั่วๆ เลยนะ 😅', 'โยนเหรียญยังดีกว่า 🪙'])
      : groupWinRate < 50
      ? pick(['แย่กว่าโยนเหรียญนิดนึง 🪙', 'เกือบแล้ว ยังไม่ถึงครึ่ง 😬', 'ลองโยนเหรียญบ้างไหม'])
      : groupWinRate < 60
      ? pick(['พอๆ กับโยนเหรียญ 🤷', 'ทายถูกครึ่งนึง ไม่แน่ใจจะดีใจไหม', '50/50 อยู่นะ ระวังด้วย'])
      : pick(['เก่งกว่าโยนเหรียญ 🎯', 'เล่นเป็น ถูกมากกว่าผิด', 'ดีนะแต่บอลยังเหลืออีกเยอะ 😏']);
    ...
    groupStatsHtml += `<div style="margin-bottom:12px;padding:10px 12px;background:var(--bg-input);border-radius:var(--radius);font-size:0.78rem">`;
    groupStatsHtml += `<div style="font-weight:700;color:var(--text-muted);margin-bottom:6px">📊 สถิติกลุ่ม (${statsLabel})</div>`;
    if (worstSlip) {
  ...
  // Headline banner — เด่น + ฮา
  html += `<div style="padding:12px 14px;margin-bottom:10px;background:var(--bg-input);border-left:4px solid var(--accent);border-radius:var(--radius);font-size:1rem;font-weight:800;color:var(--text-primary)">${headline}</div>`;
  html += groupStatsHtml;

  sorted.forEach((s, i) => {
    const isMe = s.player === state.currentPlayer;
    // Big number = the season; the month is shown underneath
    const profitColor = s.seasonProfit >= 0 ? 'var(--accent)' : 'var(--secondary)';
    const profitStr = (s.seasonProfit >= 0 ? '+' : '-') + fmtM(Math.abs(s.seasonProfit));
    const monthStr = (s.totalProfit >= 0 ? '+' : '-') + fmtM(Math.abs(s.totalProfit));
    const winStr = s.settled > 0 ? `${s.wins}/${s.settled}` : '-';
    const streakStr = s.streakVal >= 1 ? `${s.streakVal} วันติด` : '';
    const border = i === 0 && s.seasonProfit > 0 ? 'border:2px solid var(--accent);'
    ...
    if (s.carry.hasHistory) html += ` · เดือนนี้ ${monthStr}`;
    html += `</div>`;
    if (pending) {
  ...
  // (group stats rendered above — see groupStatsHtml)
...
function getMatchRoundDate(m) {
  ...
  const thaiMs = kickoffUtc(m.date).getTime() + 7 * 3600000;
...
function renderHouseDashboard() {
  ...
  const unapprovedSlips = getPeriodSlips().filter(s => s.status !== 'cancelled' && s.status !== 'approved');
  const unapprovedMatchIds = new Set(unapprovedSlips.flatMap(s => (s.picks || []).map(p => p.match_id)));
  const todayMatches = MATCHES.filter(m => {
    const t = kickoffUtc(m.date);
  ...
  if (todayMatches.length > 0) {
    ...
    const thaiDateLabel = todayMatches[0]
      ? new Date(kickoffUtc(todayMatches[0].date).getTime() + 7 * 3600000)
          .toLocaleDateString('th-TH', { day: 'numeric', month: 'short' })
      : '';
...
function getPickOutcome(pick, result) {
  ...
  if (pick.type === 'card') {
    if (!result || result.card_result == null || result.card_result === '') return null;
    const cardLine = pick.line || state.cardLines[pick.match_id];
  ...
  if (pick.type === 'champion') {
    ...
    return pick.pick === winner ? 'full' : 'loss';
  }
...
function togglePickSlips(matchId, side) {
  ...
  const filtered = getPeriodSlips().filter(s => {
